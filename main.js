/* TopDraw Services — nav scroll state, mobile drawer, scroll-reveal, subtle work-image parallax. */
(function () {
  ...
  // subtle parallax on the work images (lightweight, rAF-throttled)
  if (!reduce) {
    var pars = Array.prototype.slice.call(document.querySelectorAll('.row-media .par'));
    if (pars.length) {
      var ticking = false;
      var update = function () {
        ...
        pars.forEach(function (p) {
          ...
          var prog = (r.top + r.height / 2 - vh / 2) / vh; // -0.5..0.5 across viewport
          p.style.transform = 'translateY(' + (prog * -7).toFixed(2) + '%)';
        });
        ticking = false;
      };
      var onP = function () { if (!ticking) { ticking = true; requestAnimationFrame(update); } };
